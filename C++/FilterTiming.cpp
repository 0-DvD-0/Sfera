...
bool IsValid(float charge[], int flag [2]);
...
bool IsValid(float charge[], int Flag [2]) {
    ...
    while((Trigger<=1) && (i<CHANNELS)){

      if ((charge[i] < Q_1200_R[i] && charge[i] > Q_1200_L[i])){
...
void Parse(fs::path path){
    ...
    if (dPos!=std::string::npos){
      ...
      float base     [RESERVE];
      float amp      [RESERVE];
      float charge   [RESERVE];
      float letime   [RESERVE];
      float tetime   [RESERVE];
      float pshape   [RESERVE][1024];
      int   Flag     [2];
      ...
      tree->Branch( "Flag"    , Flag      , "Flag[2]/I"  );
